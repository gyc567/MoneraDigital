...
| `balance` | DECIMAL(32,16) | Not Null | **账户余额**（包含已扣款，不含冻结） |
...
> - 提现时，冻结的金额对其他操作（如申购）不可见
> - Safeheron 调用成功 → frozen_balance 清除，balance 已扣
> - Safeheron 调用失败或超时 → frozen_balance 解冻，balance 恢复
>
> **精妙之处**：通过 `version` 字段实现乐观锁，更新语句如 `UPDATE account SET balance = balance - ?, frozen_balance = frozen_balance - ?, version = version + 1 WHERE id = ? AND version = ? AND balance >= ?`，无需数据库行锁等待，性能高且绝对安全。
...
**Q4: 账户中的两个余额字段怎么区分？** ⚠️ **P0 关键补充**
*   **A**（冻结机制详解）：

    **账户有两个余额字段**（提现时尤为重要）：
    - `balance`：账户实际余额（已经过提现扣款，不含冻结）
    - `frozen_balance`：冻结余额（提现等待中的冻结金额）

    **用户看到的"可用余额" = balance - frozen_balance**

    **为什么需要冻结机制？**
    提现可能失败（Safeheron 故障、地址非法等），所以需要在 Safeheron 返回成功前保护金额，防止其他操作（如申购）扣除该金额。
    ...
    **流程详解**：
    ...
    用户可用余额 = 2000 - 1000 = 1000（期间无法再冻结或申购超过 1000）
      ↓
    调用 Safeheron 成功
      ↓
    扣款 + 清除冻结：
