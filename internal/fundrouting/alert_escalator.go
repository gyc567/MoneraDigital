...
func NewAlertEscalator(db *sql.DB) (*AlertEscalator, error) {
	...
	// On-chain provider SLA thresholds are 1h/6h/24h; other OPEN reasons retain
	// 1h/24h. Approval and signing remain tracked without operator alerts.
...
SELECT min((CASE WHEN threshold.reason_filter='STATUS_NOT_TERMINAL'
             THEN chain_progress.started_at
...
WHERE routing.decision='OPEN'
  ...
  AND (routing.reason_code<>'STATUS_NOT_TERMINAL'
    OR chain_progress.started_at IS NOT NULL)
  AND (routing.reason_code<>'STATUS_NOT_TERMINAL'
    OR (status_check.last_check_outcome='OBSERVED'
      AND status_check.last_observed_status IN ('BROADCASTING','CONFIRMING'))
  ...
  AND NOT EXISTS (
    ...
    WHERE alert.case_id=routing.id AND alert.alert_type='SLA_ESCALATION'
      AND alert.transition_key=CASE WHEN threshold.reason_filter='STATUS_NOT_TERMINAL'
        THEN 'sla:onchain:level:' ELSE 'sla:open:level:' END || threshold.level::text
  )`
}
...
func openCaseSLAEscalationSQL() string {
  ...
  SELECT routing.id AS case_id,routing.reason_code,threshold.reason_filter,
         ...
         chain_progress.started_at AS sla_started_at,
         floor(extract(epoch FROM now()-CASE WHEN threshold.reason_filter='STATUS_NOT_TERMINAL'
           THEN chain_progress.started_at
           ELSE COALESCE(recovery.started_at,routing.created_at)
         END))::bigint AS stuck_seconds,
  ...
  WHERE routing.decision='OPEN'
    ...
    AND (routing.reason_code<>'STATUS_NOT_TERMINAL'
      OR (chain_progress.started_at IS NOT NULL
        AND status_check.last_checked_at >= chain_progress.started_at + threshold.minimum_age))
    AND (routing.reason_code<>'STATUS_NOT_TERMINAL'
      OR status_check.last_check_outcome='ERROR'
      OR (status_check.last_check_outcome='OBSERVED'
    ...
    AND (routing.reason_code<>'STATUS_NOT_TERMINAL'
      OR (status_check.last_check_outcome='OBSERVED'
        AND status_check.last_observed_status IN ('BROADCASTING','CONFIRMING'))
      OR (status_check.last_check_outcome='ERROR'
        AND source.provider_status IN ('BROADCASTING','CONFIRMING')))
    AND NOT EXISTS (
      SELECT 1 FROM safeheron_transaction_routing_alerts alert
      WHERE alert.case_id=routing.id AND alert.alert_type='SLA_ESCALATION'
        AND alert.transition_key=CASE WHEN threshold.reason_filter='STATUS_NOT_TERMINAL'
    ...
    AND (threshold.reason_filter<>'STATUS_NOT_TERMINAL' OR NOT EXISTS (
      ...
      JOIN thresholds emitted
       ...
       AND alert.transition_key='sla:onchain:level:' || emitted.level::text
      WHERE alert.case_id=routing.id AND alert.alert_type='SLA_ESCALATION'
    ))
  ORDER BY CASE WHEN threshold.reason_filter='STATUS_NOT_TERMINAL'
             THEN chain_progress.started_at
             ELSE COALESCE(recovery.started_at,routing.created_at)
           END,
           routing.id,
...
SELECT case_id,'SLA_ESCALATION',
       CASE WHEN reason_filter='STATUS_NOT_TERMINAL'
         THEN 'sla:onchain:level:' ELSE 'sla:open:level:' END || level::text,
       severity,jsonb_strip_nulls(jsonb_build_object(
         'environment',environment,
         'case_id',case_id,
         'level',level,
         'reason_code',reason_code,
         'sla_scope',CASE WHEN reason_filter='STATUS_NOT_TERMINAL' THEN 'ONCHAIN' END,
